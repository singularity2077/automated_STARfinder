...
3. [Step 2: TIF File Deconvolution Processing](#step-2-tif-file-deconvolution-processing)
4. [Step 3: Registration and Stitching Processing Pipeline](#step-3-registration-and-stitching-processing-pipeline)
5. [Complete Running Example](#complete-running-example)
...
This pipeline consists of three main steps for processing biological image data:
...
**Important**: Each step must be executed sequentially. The next step can only begin after the previous step is completed.
...
### Script Location
`decon-sparse_block-main/HPC_run/LifToTif.sh`
...
### Script Location
`decon-sparse_block-main/HPC_run/decon1.sh`
...
**Important**: Before running `decon1.sh`, you must first create a `filelist.txt` file containing the full paths of all TIF files that need to be processed.
