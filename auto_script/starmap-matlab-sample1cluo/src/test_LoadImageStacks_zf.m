function [output_imgs, dims] = test_LoadImageStacks_zf( inputPath, sub_dir, input_dim, input_format, zrange, output_class, useGPU )
%LOADIMAGESTACKS Load image stacks for each round    
    % Suppress all warnings 
    warning('off','all');
    
    % Get directories containing all images 
    dirs = dir(fullfile(inputPath, 'round*'));
    Nround = numel(dirs);
    Nchannel = 3;
    ...
    switch output_class
        case "mat"
            ...
            for r=1:Nround
                ...
                fprintf('Found %d files in %s.\n', length(curr_files), curr_dir);
                file_num = extract(string({curr_files(:).name}), lettersPattern(2) + digitsPattern + ".tif");
                [~, order] = sort(file_num,'ascend');
                curr_files = curr_files(order);
                fprintf('Files sorted for round %d.\n', r);
                ...
                for c=1:Nchannel 
                    ...
                    disp(['Time to load TIFF: ', num2str(toc)]);
                    disp(size(curr_img, 3));
                    %disp(size(input_dim, 3));
                    % append blank layer if z doesnt match 
