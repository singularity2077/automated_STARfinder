...
%                     curr_mov = max(curr_round, [], 4);
                    ...
                    params = DFTRegister2D(fix, curr_mov, false);
                    % disp("DFTRegister success!");
                    for c=1:4
                        curr_reg = DFTApply2D(gpuArray(curr_round(:,:,:,c)), params, false);
...
%                     fprintf(obj.log, sprintf('Round %d vs. Round %d finished [time=%02f]\n', r, 1, toc));
                ...
                for r=rounds
                    ...
                    starting_apply = tic;
                    for c=1:4
                        curr_reg = DFTApply2D(curr_round(:,:,:,c), params, false);
                        curr_round(:,:,:,c) = curr_reg;
                    end
...
%             obj.registeredImages = output_reg;
        ...
        function obj = NucleiRegistration( obj, ref_dapi, move_dapi )
            ...
            if obj.useGPU
                ...
                params = DFTRegister2D(fix, mov, false);

                for c=1:4
                    curr_reg = DFTApply2D(gpuArray(round1_img(:,:,:,c)), params, false);
                ...
                tic;
                params = DFTRegister2D(ref_dapi, move_dapi, false);

                for c=1:4
                    curr_reg = DFTApply2D(round1_img(:,:,:,c), params, false);
                    round1_img(:,:,:,c) = curr_reg;
                end
        ...
        function obj = NucleiRegistrationProtein( obj, protein_folder, sub_dir, dapi_channel )
            ...
            if obj.useGPU
                ...
                for c=1:4
                    curr_reg = DFTApply2D(gpuArray(protein_imgs{c}), params, false);
...
%%%% add 2023-03-15
    ...
    %%%%%%%%%%%%%%%%%%%%
        ...
        function obj = SpotFinding( obj, varargin )
            ...
            addOptional(p,'Method',defaultMethod);
            addOptional(p,'intensityThreshold', defaultintensityThreshold);
        ...
        function obj = ReadsFiltration( obj, varargin )
            ...
            switch p.Results.mode
                ...
                case "tri"
                    obj = new_FilterReads_tri(obj, p.Results.endBases, p.Results.showPlots);
