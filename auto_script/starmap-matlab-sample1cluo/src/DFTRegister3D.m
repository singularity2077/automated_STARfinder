...
size(fixedVolume)
size(movingVolume)
disp(['Size of fixedVolume: ', mat2str(size(fixedVolume))]);
disp(['Size of movingVolume: ', mat2str(size(movingVolume))]);

if preFFT
    CC = ifftn(fixedVolume .* conj(movingVolume));
else
    disp(['Size of fixedVolume: ', num2str(size(fixedVolume))]);
    disp(['Size of movingVolume: ', num2str(size(movingVolume))]);
