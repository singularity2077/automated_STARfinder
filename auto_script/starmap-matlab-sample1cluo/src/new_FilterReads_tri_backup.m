...
% FilterReads_Duo
    ...
    cs_front = filtBases(:, 1:2);
    cs_back = filtBases(:, 3:4);
    cs_omics = filtBases(:,5);

    bases_front = new_DecodeCS(cs_front, endBases(1));
    bases_back = new_DecodeCS(cs_back, endBases(2));
    bases_omics = new_DecodeCS(cs_omics, endBases(3));
    ...
    s1 = sprintf('%f [%d / %d] percent of good reads match barcode pattern NN%s - NN%s - N%s\n',...
