...
%             obj.registeredImages = output_reg;
        ...
        function obj = NucleiRegistrationProtein( obj, protein_folder, sub_dir, dapi_channel )
            ...
            ref_dapi_path = dir(fullfile(obj.inputPath, "round01", sub_dir, "*.tif"));
            ref_dapi = new_LoadMultipageTiff(fullfile(ref_dapi_path(4).folder, ref_dapi_path(4).name), 'uint8', 'uint8', false);
	    ...
	    fprintf(sprintf('doReverse: %d\n', p.Results.doReverse));
            ...
            switch p.Results.mode
                ...
                case "duo"
                    [obj.geneToSeq, obj.seqToGene] = new_LoadCodebook(obj.inputPath, p.Results.remove_index, p.Results.doReverse);
                case "tri"
                    [obj.geneToSeq, obj.seqToGene] = new_LoadCodebook_tri(obj.inputPath, p.Results.remove_index, p.Results.doReverse);
